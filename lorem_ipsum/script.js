...
function createElementWithClasses(tag, ...classes) {
    ...
    for (let cl of classes) el.classList.add(cl);
    return el;
}

document.addEventListener('DOMContentLoaded', () => {
    let form = document.querySelector('.gen-form');
    let loremText = document.querySelector('.lorem-text');
    let amount = document.getElementById('amount');

    function generateLorem(n) {
        loremText.replaceChildren();
        for (let i = 0; i < n; ++i) {
            let rnd_text = text[Math.floor(Math.random() * text.length)];
            let lorem = createElementWithClasses('p', 'result');
            lorem.innerText = rnd_text;
            console.log(lorem);
